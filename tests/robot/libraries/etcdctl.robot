...
Put Memif Interface
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/interface/${name}
...
Put Veth Interface
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/linux/config/${AGENT_VER}/interface/${name}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put Bridge Domain
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/bd/${name}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Get Bridge Domain ID
    ...
    ${bds_dump}=    Execute On Machine    docker    curl -X GET http://localhost:9191/vpp/dump/v2/bd        #wget /vpp/dump/v2/bd
...
Put Static Fib Entry
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/bd/${bd_name}/fib/${mac}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put BFD Session
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/bfd/session/${session_name}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put BFD Authentication Key
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/bfd/auth-key/${key_name}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put BFD Echo Function
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/bfd/echo-function
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}

Get BFD Session As Json
    [Arguments]    ${node}    ${session_name}
    ${key}=               Set Variable            /vnf-agent/${node}/vpp/config/${AGENT_VER}/bfd/session/${session_name}
    ${data}=              Read Key    ${key}
    ${data}=              Set Variable If      '''${data}'''==""    {}    ${data}
    ${output}=            Evaluate             json.loads('''${data}''')    json
    [Return]              ${output}
...
Get BFD Authentication Key As Json
    ...
    ${key}=               Set Variable          /vnf-agent/${node}/vpp/config/${AGENT_VER}/bfd/auth-key/${key_name}
    ${data}=              Read Key    ${key}
    ${data}=              Set Variable If      '''${data}'''==""    {}    ${data}
    ${output}=            Evaluate             json.loads('''${data}''')    json
    [Return]              ${output}
...
Get BFD Echo Function As Json
    [Arguments]    ${node}
    ${key}=               Set Variable          /vnf-agent/${node}/vpp/config/${AGENT_VER}/bfd/echo-function
    ${data}=              Read Key    ${key}
    ${data}=              Set Variable If      '''${data}'''==""    {}    ${data}
    ${output}=            Evaluate             json.loads('''${data}''')    json
    [Return]              ${output}
...
Put ACL TCP
    ...
    ${uri}=               Set Variable          /vnf-agent/${node}/vpp/config/${AGENT_VER}/acl/${acl_name}
    ${data}=              Replace Variables             ${data}
...
Get ACL As Json
    ...
    ${key}=               Set Variable          /vnf-agent/${node}/vpp/config/${AGENT_VER}/acl/${acl_name}
    ${data}=              Read Key    ${key}
...
Get All ACL As Json
    [Arguments]           ${node}
    ${key}=               Set Variable          /vnf-agent/${node}/vpp/config/${AGENT_VER}/acl
...
Delete ACL
    [Arguments]    ${node}    ${name}
    ${uri}=      Set Variable    /vnf-agent/${node}/vpp/config/${AGENT_VER}/acl/${name}
    ${out}=      Delete key    ${uri}
    [Return]    ${out}
...
Put Linux Route
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/linux/config/${AGENT_VER}/route/${routename}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put ARP
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/arp/${interface}/${ipv4}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}

Get ARP As Json
    [Arguments]           ${node}  ${interface}
    ${key}=               Set Variable          /vnf-agent/${node}/vpp/config/${AGENT_VER}/arp/${interface}
    ${data}=              Read Key    ${key}
    ${data}=              Set Variable If      '''${data}'''==""    {}    ${data}
    ${output}=            Evaluate             json.loads('''${data}''')     json
    [Return]              ${output}
...
Set L4 Features On Node
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/l4/features/feature
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put Application Namespace
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/l4/namespaces/${id}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put Linux ARP With Namespace
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/linux/config/${AGENT_VER}/arp/${arpname}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put L2XConnect
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/xconnect/${rx_if}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put Local SID
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/srv6/localsid/${localsidName}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put SRv6 Policy
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/srv6/policy/${name}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put SRv6 Policy Segment
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/srv6/policy/${policyName}/segment/${name}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
...
Put SRv6 Steering
    ...
    ${uri}=               Set Variable                  /vnf-agent/${node}/vpp/config/${AGENT_VER}/srv6/steering/${name}
    ${data}=              Replace Variables             ${data}
    Put Json     ${uri}    ${data}
