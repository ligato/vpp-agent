...
Create Route On ${node} With IP ${dst-ip}/${dst-mask} With Next Hop ${next-hop} And Vrf Id ${id}
    ${data}=        OperatingSystem.Get File    ${CURDIR}/../../robot/resources/static_route.json
    ${data}=        replace variables           ${data}
    ${uri}=         Set Variable                /vnf-agent/${node}/vpp/config/${AGENT_VER}/route/vrf/${id}/dst/${dst-ip}/${dst-mask}/gw/${next-hop}
