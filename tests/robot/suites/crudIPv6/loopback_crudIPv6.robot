...
Add Loopback1 Interface
    ...
    Put Loopback Interface With IP    node=agent_vpp_1    name=${NAME_LOOP1}
    ...    mac=${MAC_LOOP1}    ip=${IP_LOOP1}    prefix=${PREFIX}    mtu=${MTU}    enabled=true
...
Check Loopback1 Is Created
    ...
    Wait Until Keyword Succeeds   ${WAIT_TIMEOUT}   ${SYNC_SLEEP}
    ...    vpp_api: Check Loopback Interface State    agent_vpp_1    ${NAME_LOOP1}
    ...    enabled=1     mac=${MAC_LOOP1}    mtu=${MTU}  ipv6=${IP_LOOP1}/${PREFIX}

Add Loopback2 Interface
    vpp_term: Interface Not Exists  node=agent_vpp_1    mac=${MAC_LOOP2}
    Put Loopback Interface With IP    node=agent_vpp_1     name=${NAME_LOOP2}
    ...    mac=${MAC_LOOP2}    ip=${IP_LOOP2}    prefix=${PREFIX}    mtu=${MTU}    enabled=true

Check Loopback2 Is Created
    Wait Until Keyword Succeeds   ${WAIT_TIMEOUT}   ${SYNC_SLEEP}
    ...    vpp_term: Interface Is Created    node=agent_vpp_1    mac=${MAC_LOOP2}
    Wait Until Keyword Succeeds   ${WAIT_TIMEOUT}   ${SYNC_SLEEP}
    ...    vpp_api: Check Loopback Interface State    agent_vpp_1    ${NAME_LOOP2}
    ...    enabled=1     mac=${MAC_LOOP2}    mtu=${MTU}    ipv6=${IP_LOOP2}/${PREFIX}
...
Update Loopback1
    Put Loopback Interface With IP    node=agent_vpp_1     name=${NAME_LOOP1}
    ...    mac=${MAC_LOOP1_2}    ip=${IP_LOOP1_2}    prefix=${PREFIX}    mtu=${MTU}    enabled=true
    Wait Until Keyword Succeeds   ${WAIT_TIMEOUT}   ${SYNC_SLEEP}
    ...
    Wait Until Keyword Succeeds   ${WAIT_TIMEOUT}   ${SYNC_SLEEP}
    ...    vpp_api: Check Loopback Interface State    agent_vpp_1    ${NAME_LOOP1}
    ...    enabled=1     mac=${MAC_LOOP1_2}    mtu=${MTU}    ipv6=${IP_LOOP1_2}/${PREFIX}
