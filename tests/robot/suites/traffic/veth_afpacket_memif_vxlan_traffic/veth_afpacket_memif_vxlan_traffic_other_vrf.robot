...
Setup Interfaces
    ...
    sleep    2
    # temporarily START
    ${int1}=    vpp_ctl: Get Interface Internal Name    agent_vpp_1    vpp1_memif1
    ${int3}=    vpp_ctl: Get Interface Internal Name    agent_vpp_1    vpp1_vxlan1
    vpp_term: Issue Command    node=agent_vpp_1    command=sh int addr
    vpp_term: Issue Command    node=agent_vpp_1    command=show bridge-domain 1 detail
    vpp_term: Issue Command    node=agent_vpp_1    command=show vxlan tunnel
    vpp_term: Issue Command    node=agent_vpp_1    command=create vxlan tunnel src 192.168.1.1 dst 192.168.1.2 vni 5 del
    vpp_term: Issue Command    node=agent_vpp_1    command=set int ip address del ${int1} all
    vpp_term: Issue Command    node=agent_vpp_1    command=ip table add 1
    vpp_term: Issue Command    node=agent_vpp_1    command=set int ip table host-vpp1_veth2 1
    vpp_term: Issue Command    node=agent_vpp_1    command=set int ip table ${int1} 1
    vpp_term: Issue Command    node=agent_vpp_1    command=set int ip address ${int1} 192.168.1.1/24
    ...
    vpp_term: Issue Command    node=agent_vpp_1    command=set int ip table ${int3} 1
    vpp_term: Issue Command    node=agent_vpp_1    command=set interface l2 bridge ${int3} 1
    vpp_term: Issue Command    node=agent_vpp_1    command=set bridge-domain learn 1
    # temporarily END
...
Check Ping From VPP1 to VPP2
    ...
    Ping On agent_vpp_1 With IP 192.168.1.2, Source memif0/1
