...
Create Linux Routes
    ...
    ...    ip=${IP_4}    prefix=128    next_hop=${IP_2}
    Put Linux Route    node=agent_vpp_1    namespace=ns3    interface=ns3_veth3
    ...    ip=${IP_1}    prefix=128    next_hop=${IP_3}
    # Enable forwarding in namespace ns2
    Execute In Container    agent_vpp_1    ip netns exec ns2 sysctl -w net.ipv6.conf.all.forwarding=1
...
Ping in namespace via interface
    ...
    ${out}=    Execute In Container    ${node}    ip netns exec ${namespace} ping6 -c 5 -I ${interface} ${ip}
    Should Contain     ${out}    from ${ip}
    Should Not Contain    ${out}    100% packet loss
