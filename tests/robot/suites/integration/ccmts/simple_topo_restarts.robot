...
Pod_To_Pod_Ping
    ...
    ${stdout} =    KubeEnv.Run_Finite_Command_In_Pod    ping -c 5 ${cn_infra_ip}    ssh_session=${vswitch_connection}
    BuiltIn.Should_Contain   ${stdout}    5 received, 0% packet loss
    ${stdout} =    KubeEnv.Run_Finite_Command_In_Pod    ping -c 5 ${vswitch_ip}    ssh_session=${cn_infra_connection}
    BuiltIn.Should_Contain   ${stdout}    5 received, 0% packet loss
...
Pod_To_Pod_Udp
    ...
    ${vswitch_stdout} =    KubeEnv.Stop_Infinite_Command_In_Pod    ssh_session=${vswitch_connection}
    BuiltIn.Should_Contain   ${vswitch_stdout}    ${text}
    [Teardown]    Teardown_Hosts_Connections
...
Pod_To_Pod_Tcp
    ...
    [Setup]    Setup_Hosts_Connections
    ${text} =    BuiltIn.Set_Variable    Text to be received
    KubeEnv.Run_Finite_Command_In_Pod    cd; echo "${text}" > some.file    ssh_session=${cn_infra_connection}
    KubeEnv.Init_Infinite_Command_in_Pod    nc -l -p 4444    ssh_session=${vswitch_connection}
    KubeEnv.Init_Infinite_Command_in_Pod    cd; nc ${vswitch_ip} 4444 < some.file    ssh_session=${cn_infra_connection}
    ${vswitch_stdout} =    KubeEnv.Stop_Infinite_Command_In_Pod    ssh_session=${vswitch_connection}
    BuiltIn.Should_Contain   ${vswitch_stdout}    ${text}
    ${cn_infra_stdout} =    KubeEnv.Stop_Infinite_Command_In_Pod    ssh_session=${cn_infra_connection}
    [Teardown]    Teardown_Hosts_Connections
...
Host_To_Pod_Ping
    ...
    [Setup]    Setup_Hosts_Connections
    ${stdout} =    SshCommons.Switch_And_Execute_Command    ${testbed_connection}    ping -c 5 ${vswitch_ip}
    BuiltIn.Should_Contain   ${stdout}    5 received, 0% packet loss
    ${stdout} =    SshCommons.Switch_And_Execute_Command    ${testbed_connection}    ping -c 5 ${cn_infra_ip}
    BuiltIn.Should_Contain   ${stdout}    5 received, 0% packet loss
    [Teardown]    Teardown_Hosts_Connections
...
Host_To_Pod_Udp
    ...
    KubeEnv.Init_Infinite_Command_in_Pod    nc -u ${vswitch_ip} 7000    ssh_session=${testbed_connection}
    ${text} =    BuiltIn.Set_Variable    Text to be received
    SSHLibrary.Write    ${text}
    ${cn_infra_stdout} =    KubeEnv.Stop_Infinite_Command_In_Pod    ssh_session=${testbed_connection}    prompt=$
    ${vswtich_stdout} =    KubeEnv.Stop_Infinite_Command_In_Pod    ssh_session=${vswitch_connection}
...
Host_To_Pod_Tcp
    ...
    KubeEnv.Init_Infinite_Command_in_Pod    nc -l -p 4444    ssh_session=${vswitch_connection}
    KubeEnv.Init_Infinite_Command_in_Pod    cd; nc ${vswitch_ip} 4444 < some.file    ssh_session=${testbed_connection}
    ${vswitch_stdout} =    KubeEnv.Stop_Infinite_Command_In_Pod    ssh_session=${vswitch_connection}
    BuiltIn.Should_Contain   ${vswitch_stdout}    ${text}
    ${cn_infra_stdout} =    KubeEnv.Stop_Infinite_Command_In_Pod    ssh_session=${testbed_connection}    prompt=$
    [Teardown]    Teardown_Hosts_Connections
