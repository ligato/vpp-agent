...
type IfaceMetadataIndex interface {
	...
	WatchInterfaces(subscriber infra.PluginName, channel chan<- IfaceMetadataDto)
}
...
// WatchInterfaces allows to subscribe to watch for changes in the mapping
// if interface metadata.
func (ifmx *ifaceMetadataIndex) WatchInterfaces(subscriber infra.PluginName, channel chan<- IfaceMetadataDto) {
