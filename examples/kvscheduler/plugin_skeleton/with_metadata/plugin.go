...
/*******************************************************************************
 ...
 and a metadata index-map to build your own plugin in 6 steps:
  ...
  2. define model for each of your value types in model/keys.go, which will be
     ...
     instances
  3. use "metaidx" as a template for implementing your own index maps with
     secondary lookups over your metadata
      - note: if you only to keep an integer SB handle with each value instance,
              you may instead re-use the ligato/vpp-agent/pkg/idxvpp map, even
              if SB is not actually VPP (metadata type is a structure called
              "OnlyIndex") - for an example see descriptor for VPP bridge domains,
              located in ligato/vpp-agent/plugins/vpp/l2plugin/descriptor
  4. for each of your value types, including derived types (with the exception
   ...
   5. register all your descriptors with KVScheduler in the Init method of the
      plugin
   6. finally, remove comments whose purpose was solely to guide you through
