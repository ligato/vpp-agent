...
import (
	...
	"github.com/ligato/vpp-agent/examples/kvscheduler/plugin_skeleton/with_metadata/descriptor/adapter"
...
func NewSkeletonDescriptor(log logging.PluginLogger) *kvs.KVDescriptor {
	...
	typedDescr := &adapter.SkeletonDescriptor{
		...
		WithMetadata:         true,
		MetadataMapFactory:   descrCtx.MetadataMapFactory,
...
// MetadataMapFactory is a factory for index-map with skeleton metadata.
func (d *SkeletonDescriptor) MetadataMapFactory() idxmap.NamedMappingRW {
	return metaidx.NewSkeletonIndex(d.log, "skeleton-index")
}
...
func (d *SkeletonDescriptor) Create(key string, value *model.ValueSkeleton) (metadata *metaidx.SkeletonMetadata, err error) {
	return nil, nil
}

// Delete removes an existing value.
func (d *SkeletonDescriptor) Delete(key string, value *model.ValueSkeleton, metadata *metaidx.SkeletonMetadata) error {
	return nil
}

// Update updates existing value.
func (d *SkeletonDescriptor) Update(key string, old, new *model.ValueSkeleton, oldMetadata *metaidx.SkeletonMetadata) (newMetadata *metaidx.SkeletonMetadata, err error) {
	return nil, nil
}

// UpdateWithRecreate returns true if value update requires full re-creation.
func (d *SkeletonDescriptor) UpdateWithRecreate(key string, old, new *model.ValueSkeleton, metadata *metaidx.SkeletonMetadata) bool {
	return false
}
