...
func (plugin *InterfaceStateUpdater) Init(ctx context.Context,
	...
	plugin.Info("Initializing InterfaceStateUpdater")
...
func (plugin *InterfaceStateUpdater) subscribeVPPNotifications() error {
	...
	plugin.Debug("wantInterfaceEventsReply: ", wantInterfaceEventsReply, " ", err)
	if err != nil {
		return err
	}
	...
	plugin.Debug("wantStatsReply: ", wantStatsReply, " ", err)
	if err != nil {
		return err
	}
...
func (plugin *InterfaceStateUpdater) watchVPPNotifications(ctx context.Context) {
	...
	if plugin.notifChan != nil {
		plugin.Info("watchVPPNotifications starting")
	} else {
		plugin.Error("watchVPPNotifications will not start")
		return
	}
	...
	for {
		...
		case msg := <-plugin.notifChan:
			...
			default:
				plugin.WithFields(logging.Fields{"MessageName": msg.GetMessageName()}).Debug("Ignoring unknown VPP notification")
			}
...
func (plugin *InterfaceStateUpdater) processIfStateNotification(notif *interfaces.SwInterfaceEvent) {
	...
	if !found {
		plugin.WithField("swIfIndex", notif.SwIfIndex).
	...
	plugin.WithFields(logging.Fields{"ifName": ifState.Name, "swIfIndex": notif.SwIfIndex, "AdminUpDown": notif.AdminUpDown,
...
func (plugin *InterfaceStateUpdater) processIfCounterNotification(counter *interfaces.VnetInterfaceSimpleCounters) {
	...
	for i := uint32(0); i < counter.Count; i++ {
		...
		if !found {
			plugin.WithField("swIfIndex", swIfIndex).
...
func (plugin *InterfaceStateUpdater) updateIfStateDetails(ifDetails *interfaces.SwInterfaceDetails) {
	...
	if !found {
		plugin.WithField("swIfIndex", ifDetails.SwIfIndex).
