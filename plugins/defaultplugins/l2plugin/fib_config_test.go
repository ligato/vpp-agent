...
func TestFIBConnectConfiguratorInit(t *testing.T) {
	...
	plugin := &l2plugin.FIBConfigurator{
		...
		GoVppmux:      connection,
		SwIfIndexes:   ifaceidx.NewSwIfIndex(nametoidx.NewNameToIdx(logrus.DefaultLogger(), "fib-if", "if", ifaceidx.IndexMetadata)),
		BdIndexes:     bdidx.NewBDIndex(nametoidx.NewNameToIdx(logrus.DefaultLogger(), "fib-bd", "if", ifaceidx.IndexMetadata)),
		IfToBdIndexes: nametoidx.NewNameToIdx(logrus.DefaultLogger(), "fib-ifToBd", "ifToBd", ifaceidx.IndexMetadata),
		FibIndexes:    bdidx.NewFIBIndex(nametoidx.NewNameToIdx(logrus.DefaultLogger(), "fib", "fib_indexes", nil)),
		Stopwatch:     nil,
	}
	// Test init
	err = plugin.Init()
...
func fibTestSetup(t *testing.T) (*vppcallmock.TestCtx, *core.Connection, *l2plugin.FIBConfigurator, ifaceidx.SwIfIndexRW, bdidx.BDIndexRW) {
	RegisterTestingT(t)
	ctx := &vppcallmock.TestCtx{
		MockVpp: &mock.VppAdapter{},
	}
	...
	// Interface indices
	swIfIndexes := ifaceidx.NewSwIfIndex(nametoidx.NewNameToIdx(log, "fib-if", "fib-if", nil))
	bdIndexes := bdidx.NewBDIndex(nametoidx.NewNameToIdx(log, "fib-bd", "fib-bd", nil))
	// Configurator
	plugin := &l2plugin.FIBConfigurator{
		Log:           logging.ForPlugin("test-log", logrus.NewLogRegistry()),
		GoVppmux:      connection,
		...
		BdIndexes:     bdIndexes,
		IfToBdIndexes: nametoidx.NewNameToIdx(logrus.DefaultLogger(), "ifToBd", "ifToBd", ifaceidx.IndexMetadata),
		FibIndexes:    bdidx.NewFIBIndex(nametoidx.NewNameToIdx(logrus.DefaultLogger(), "fib", "fib_indexes", nil)),
		Stopwatch:     nil,
	}
	err = plugin.Init()
...
func getBdMetaWithInterfaces(bdName string, ifs, configured []string) *bdidx.BdMetadata {
