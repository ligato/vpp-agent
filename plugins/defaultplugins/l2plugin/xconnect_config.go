...
func (plugin *XConnectConfigurator) Init() (err error) {
	...
	plugin.stopwatch = timer.NewStopwatch("XConnectConfigurator", plugin.Log)
...
func (plugin *XConnectConfigurator) ConfigureXConnectPair(xConnectPairInput *l2.XConnectPairs_XConnectPair) error {
	...
	if rxFound && txFound {
		...
		err := vppcalls.VppSetL2XConnect(receiveInterfaceIndex, transmitInterfaceIndex, plugin.Log, plugin.vppChan, plugin.stopwatch)
		if err != nil {
...
func (plugin *XConnectConfigurator) deleteL2XConnectPair(rxIf, txIf string) error {
	...
	err := vppcalls.VppUnsetL2XConnect(receiveInterfaceIndex, transmitInterfaceIndex, plugin.Log, plugin.vppChan, plugin.stopwatch)
	if err != nil {
