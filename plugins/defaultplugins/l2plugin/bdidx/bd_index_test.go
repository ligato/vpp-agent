...
func TestIndexMetadata(t *testing.T) {
	...
	result = bdidx.IndexMetadata(bdidx.NewBDMetadata(bridgeDomain, nil))
...
func TestRegisterAndUnregisterName(t *testing.T) {
	...
	bdIndex.RegisterName(bridgeDomains[0].Name, idx0, bdidx.NewBDMetadata(bridgeDomains[0], nil))
	names := nameToIdx.ListNames()
...
func TestUpdateMetadata(t *testing.T) {
	...
	success := bdIndex.UpdateMetadata(bd.Name, bdidx.NewBDMetadata(bd, []string{ifaceAName, ifaceBName}))
	...
	bdIndex.RegisterName(bd.Name, idx0, bdidx.NewBDMetadata(bd, []string{ifaceAName, ifaceBName}))
	var names []string
	names = nameToIdx.ListNames()
	Expect(names).To(HaveLen(1))
	...
	bdData, ok := metadata.(*bdidx.BdMetadata)
	Expect(ok).To(BeTrue())
	...
	success = bdIndex.UpdateMetadata(bdUpdt1.Name, bdidx.NewBDMetadata(bdUpdt1, []string{ifaceCName}))
	...
	success = bdIndex.UpdateMetadata(bdUpdt2.Name, bdidx.NewBDMetadata(bdUpdt2, []string{ifaceDName}))
	Expect(success).To(BeTrue())
...
func TestLookupIndex(t *testing.T) {
	...
	bdIndex.RegisterName(bridgeDomains[0].Name, idx0, bdidx.NewBDMetadata(bridgeDomains[0], []string{ifaceAName, ifaceBName}))
...
func TestLookupByIfaceName(t *testing.T) {
	...
	for _, bridgeDomain := range bridgeDomains {
		if bridgeDomain.Name == bdName0 {
			bdIndex.RegisterName(bridgeDomain.Name, idx0, bdidx.NewBDMetadata(bridgeDomain, []string{ifaceAName, ifaceBName}))
		} else if bridgeDomain.Name == bdName1 {
			bdIndex.RegisterName(bridgeDomain.Name, idx1, bdidx.NewBDMetadata(bridgeDomain, []string{ifaceCName}))
		} else {
			bdIndex.RegisterName(bridgeDomain.Name, idx2, bdidx.NewBDMetadata(bridgeDomain, []string{ifaceDName}))
		}
	}
