...
import (
	...
	"github.com/ligato/vpp-agent/plugins/defaultplugins/l2plugin/vppcalls/iface"
)
...
func VppAddBridgeDomain(bdIdx uint32, bridgeDomain *l2.BridgeDomains_BridgeDomain, log logging.Logger,
	vppChan iface.VPPChannel, timeLog measure.StopWatchEntry) error {
...
func VppUpdateBridgeDomain(oldBdIdx uint32, newBdIdx uint32, newBridgeDomain *l2.BridgeDomains_BridgeDomain, log logging.Logger,
	vppChan iface.VPPChannel, stopwatch *measure.Stopwatch) error {
...
func VppDeleteBridgeDomain(bdIdx uint32, log logging.Logger, vppChan iface.VPPChannel, timeLog measure.StopWatchEntry) error {
	// BridgeDomainAddDel time measurement
	start := time.Now()
	defer func() {
		if timeLog != nil {
