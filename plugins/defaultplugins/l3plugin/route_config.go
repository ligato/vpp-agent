...
func (plugin *RouteConfigurator) Init() (err error) {
	plugin.Debug("Initializing L3 plugin")
...
func (plugin *RouteConfigurator) ConfigureRoute(config *l3.StaticRoutes_Route, vrfFromKey string) error {
	plugin.Infof("Creating new route %v -> %v", config.DstIpAddr, config.NextHopAddr)
	...
	if intVrfFromKey != int(config.VrfId) {
		plugin.Warnf("VRF index from key (%v) and from config (%v) does not match, using value from the key",
	...
	route, err := TransformRoute(config, plugin.SwIfIndexes, plugin.Logger)
	if err != nil {
		return err
	}
	...
	if route != nil {
		...
		plugin.Infof("Route %v registered", routeIdentifier)
	}
...
func (plugin *RouteConfigurator) ModifyRoute(newConfig *l3.StaticRoutes_Route, oldConfig *l3.StaticRoutes_Route, vrfFromKey string) error {
	plugin.Infof("Modifying route %v -> %v ", oldConfig.DstIpAddr, oldConfig.NextHopAddr)
	...
	if intVrfFromKey != int(newConfig.VrfId) {
		...
		plugin.Warnf("VRF index was changed to (%v) while the VRF in the key is (%v), using value from the key",
	...
	newRoute, err := TransformRoute(newConfig, plugin.SwIfIndexes, plugin.Logger)
	if err != nil {
		return err
	}
	// Transform old route data
	oldRoute, err := TransformRoute(oldConfig, plugin.SwIfIndexes, plugin.Logger)
	if err != nil {
		return err
	}
	...
	plugin.Infof("Old route %v unregistered", oldRouteIdentifier)
	...
	plugin.RouteIndexSeq++
	plugin.Infof("New route %v registered", newRouteIdentifier)
...
func (plugin *RouteConfigurator) DeleteRoute(config *l3.StaticRoutes_Route) (wasError error) {
	plugin.Infof("Removing route %v -> %v", config.DstIpAddr, config.NextHopAddr)
	// Transform route data
	route, err := TransformRoute(config, plugin.SwIfIndexes, plugin.Logger)
	if err != nil {
		return err
	...
	plugin.Infof("Route %v unregistered", routeIdentifier)
