...
func TestAddArpCached(t *testing.T) {
	// Setup
	_, goVppMux, plugin, _ := arpTestSetup(t)
	defer arpTestTeardown(goVppMux, plugin)
...
func TestAddArpAndResolve(t *testing.T) {
	// Setup
	ctx, goVppMux, plugin, ifIndexes := arpTestSetup(t)
	defer arpTestTeardown(goVppMux, plugin)
...
func TestDeleteArpNonExistingIntf(t *testing.T) {
	...
	numArps := len(plugin.GetArpIndexes().GetMapping().ListNames())
	...
	Expect(err).ShouldNot(HaveOccurred())
	Expect(plugin.GetArpIndexes().GetMapping().ListNames()).To(HaveLen(numArps))
...
func TestDeleteArpCached(t *testing.T) {
	...
	plugin.GetArpCache().RegisterName("arp-iface-tap3-59:6C:DE:AD:00:03-dead::01", 1, arpEntry)
	numArps := len(plugin.GetArpCache().GetMapping().ListNames())
	err := plugin.DeleteArp(arpEntry)
	Expect(err).ShouldNot(HaveOccurred())
	Expect(plugin.GetArpCache().GetMapping().ListNames()).To(HaveLen(numArps - 1))
	Expect(plugin.GetArpDeleted().GetMapping().ListNames()).To(HaveLen(0))
}
...
func TestDeleteArp(t *testing.T) {
	...
	plugin.GetArpIndexes().RegisterName("arp-iface-tap1-59:6C:DE:AD:00:01-192.168.10.21", 1, arpEntry)
	numArps := len(plugin.GetArpIndexes().GetMapping().ListNames())
	...
	Expect(err).ShouldNot(HaveOccurred())
	Expect(plugin.GetArpIndexes().GetMapping().ListNames()).To(HaveLen(numArps - 1))
}
...
func TestChangeArp(t *testing.T) {
	...
	// add arp
	arpEntryOld := &l3.ArpTable_ArpEntry{
		Interface:   "tap1",
		IpAddress:   "192.168.10.21",
		PhysAddress: "59:6C:DE:AD:00:01",
		Static:      true,
	}
	arpEntryNew := &l3.ArpTable_ArpEntry{
		Interface:   "tap2",
		IpAddress:   "192.168.10.22",
		PhysAddress: "59:6C:DE:AD:00:02",
		Static:      true,
	}
	plugin.GetArpIndexes().RegisterName("arp-iface-tap1-59:6C:DE:AD:00:01-192.168.10.21", 1, arpEntryOld)
	numArps := len(plugin.GetArpIndexes().GetMapping().ListNames())

	ctx.MockVpp.MockReply(&ip.IPNeighborAddDelReply{})
	ctx.MockVpp.MockReply(&ip.IPNeighborAddDelReply{})
	err := plugin.ChangeArp(arpEntryNew, arpEntryOld)
	Expect(err).To(BeNil())
	Expect(err).ShouldNot(HaveOccurred())
	arps := plugin.GetArpIndexes().GetMapping().ListNames()
	Expect(arps).To(HaveLen(numArps))
...
func TestArpResolveCreatedInterface(t *testing.T) {
	...
	arpEntry1 := &l3.ArpTable_ArpEntry{
		Interface:   "tap3",
		IpAddress:   "dead::01",
		PhysAddress: "59:6C:DE:AD:00:03",
		Static:      false,
	}
	arpEntry2 := &l3.ArpTable_ArpEntry{
		Interface:   "tap4",
		IpAddress:   "dead::02",
		PhysAddress: "59:6C:DE:AD:00:04",
		Static:      false,
	...
	plugin.GetArpCache().RegisterName("arp-iface-tap3-59:6C:DE:AD:00:03-dead::01", 1, arpEntry1)
	Expect(plugin.GetArpCache().GetMapping().ListNames()).To(HaveLen(1))
	Expect(plugin.GetArpDeleted().GetMapping().ListNames()).To(HaveLen(0))

	ctx.MockVpp.MockReply(&ip.IPNeighborAddDelReply{})
	ifIndexes.GetMapping().RegisterName("tap3", 1, arpEntry1)
	err := plugin.ResolveCreatedInterface("tap3")
	Expect(err).ShouldNot(HaveOccurred())
	...
	plugin.GetArpIndexes().RegisterName("arp-iface-tap4-59:6C:DE:AD:00:04-dead::02", 2, arpEntry2)
	plugin.GetArpDeleted().RegisterName("arp-iface-tap4-59:6C:DE:AD:00:04-dead::02", 2, arpEntry2)
	Expect(plugin.GetArpCache().GetMapping().ListNames()).To(HaveLen(0))
	Expect(plugin.GetArpDeleted().GetMapping().ListNames()).To(HaveLen(1))

	ctx.MockVpp.MockReply(&ip.IPNeighborAddDelReply{})
	ifIndexes.GetMapping().RegisterName("tap4", 2, arpEntry2)
...
// ARP Test Setup
func arpTestSetup(t *testing.T) (*vppcallmock.TestCtx, *mock.GoVPPMux, *l3plugin.ArpConfigurator, ifaceidx.SwIfIndex) {
	RegisterTestingT(t)
...
// Test Teardown
func arpTestTeardown(goVppMux *mock.GoVPPMux, plugin *l3plugin.ArpConfigurator) {
	goVppMux.Close()
