...
import (
	...
	vpp_nat "github.com/ligato/vpp-agent/plugins/vpp/binapi/vpp2001_379/nat"
	...
	"github.com/ligato/vpp-agent/plugins/vpp/natplugin/vppcalls/vpp2001_379"
...
func TestDNATDump(t *testing.T) {
	...
	ctx.MockVpp.MockReply(
		&vpp_nat.Nat44StaticMappingDetails{
			...
			LocalPort:         8080,
			ExternalPort:      80,
			ExternalSwIfIndex: vpp2001_379.NoInterface,
			VrfID:             1,
			Tag:               "DNAT 1",
			Flags:             vpp_nat.NAT_IS_TWICE_NAT,
		},
	...
	ctx.MockVpp.MockReply(
		&vpp_nat.Nat44IdentityMappingDetails{
			...
			SwIfIndex: vpp2001_379.NoInterface,
			VrfID:     1,
			Tag:       "DNAT 3",
		},
		&vpp_nat.Nat44IdentityMappingDetails{
...
func natTestSetup(t *testing.T) (*vppcallmock.TestCtx, vppcalls.NatVppAPI, ifaceidx.IfaceMetadataIndexRW, idxmap.NamedMappingRW) {
	...
	natHandler := vpp2001_379.NewNatVppHandler(ctx.MockChannel, swIfIndexes, dhcpIndexes, log)
