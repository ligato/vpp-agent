...
import (
	...
	vpp_nat "go.ligato.io/vpp-agent/v3/plugins/vpp/binapi/vpp2001_324/nat"
	...
	"go.ligato.io/vpp-agent/v3/plugins/vpp/natplugin/vppcalls/vpp2001_324"
...
func TestDNATDump(t *testing.T) {
	...
	ctx.MockVpp.MockReply(
		&vpp_nat.Nat44StaticMappingDetails{
			...
			LocalPort:         8080,
			ExternalPort:      80,
			ExternalSwIfIndex: vpp2001_324.NoInterface,
			VrfID:             1,
			Tag:               "DNAT 1",
			Flags:             vpp_nat.NAT_IS_TWICE_NAT,
		},
	...
	ctx.MockVpp.MockReply(
		&vpp_nat.Nat44IdentityMappingDetails{
			...
			SwIfIndex: vpp2001_324.NoInterface,
			VrfID:     1,
			Tag:       "DNAT 3",
		},
		&vpp_nat.Nat44IdentityMappingDetails{
...
func natTestSetup(t *testing.T) (*vppmock.TestCtx, vppcalls.NatVppAPI, ifaceidx.IfaceMetadataIndexRW, idxmap.NamedMappingRW) {
	...
	natHandler := vpp2001_324.NewNatVppHandler(ctx.MockChannel, swIfIndexes, dhcpIndexes, log)
