...
import (
	...
	"go.ligato.io/vpp-agent/v2/proto/ligato/vpp-agent/vpp/interfaces"
)
...
func NewSpanDescriptor(typedDescriptor *SpanDescriptor) *KVDescriptor {
	adapter := &SpanDescriptorAdapter{descriptor: typedDescriptor}
...
func (da *SpanDescriptorAdapter) ValueComparator(key string, oldValue, newValue proto.Message) bool {
	typedOldValue, err1 := castSpanValue(key, oldValue)
	typedNewValue, err2 := castSpanValue(key, newValue)
...
func (da *SpanDescriptorAdapter) Validate(key string, value proto.Message) (err error) {
	typedValue, err := castSpanValue(key, value)
	if err != nil {
		return err
	}
...
func (da *SpanDescriptorAdapter) Update(key string, oldValue, newValue proto.Message, oldMetadata Metadata) (newMetadata Metadata, err error) {
	oldTypedValue, err := castSpanValue(key, oldValue)
	if err != nil {
		return nil, err
	}
	newTypedValue, err := castSpanValue(key, newValue)
	if err != nil {
		return nil, err
	}
	typedOldMetadata, err := castSpanMetadata(key, oldMetadata)
	if err != nil {
		return nil, err
	}
...
func (da *SpanDescriptorAdapter) Delete(key string, value proto.Message, metadata Metadata) error {
	...
	typedMetadata, err := castSpanMetadata(key, metadata)
	if err != nil {
		return err
	}
...
func (da *SpanDescriptorAdapter) Retrieve(correlate []KVWithMetadata) ([]KVWithMetadata, error) {
	var correlateWithType []SpanKVWithMetadata
	for _, kvpair := range correlate {
		typedValue, err := castSpanValue(kvpair.Key, kvpair.Value)
		if err != nil {
			continue
		}
		typedMetadata, err := castSpanMetadata(kvpair.Key, kvpair.Metadata)
		if err != nil {
			continue
		}
...
func castSpanValue(key string, value proto.Message) (*vpp_interfaces.Span, error) {
	typedValue, ok := value.(*vpp_interfaces.Span)
	if !ok {
...
func castSpanMetadata(key string, metadata Metadata) (interface{}, error) {
	if metadata == nil {
		return nil, nil
	}
