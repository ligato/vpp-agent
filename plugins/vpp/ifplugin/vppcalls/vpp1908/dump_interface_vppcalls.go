...
func (h *InterfaceVppHandler) DumpInterfaces2(ifIdxs ...uint32) (map[uint32]*vppcalls.InterfaceDetails, error) {
	ifaces, err := h.dumpInterfaces(ifIdxs...)
	if err != nil {
		return nil, err
	}

	// check all requested interface indexes are in dump
	for _, ifIdx := range ifIdxs {
		if _, ok := ifaces[ifIdx]; !ok {
			//h.log.Warnf("interface %v not found in dump", ifIdx)
			return nil, fmt.Errorf("interface %v not found in dump", ifIdx)
		}
	}
	...
	err = h.dumpIPAddressDetails(ifaces, false, dhcpClients)
	if err != nil {
		return nil, err
	}
	err = h.dumpIPAddressDetails(ifaces, true, dhcpClients)
	if err != nil {
		return nil, err
	}
	...
	for _, ifData := range ifaces {
		...
		if ok {
			...
			ifWithIP, ok := ifaces[ifWithIPIdx]
			if ok {
	...
	err = h.dumpMemifDetails(ifaces)
	if err != nil {
		return nil, err
	}

	err = h.dumpTapDetails(ifaces)
	if err != nil {
		return nil, err
	}

	err = h.dumpIPSecTunnelDetails(ifaces)
	if err != nil {
		return nil, err
	}

	err = h.dumpVmxNet3Details(ifaces)
	if err != nil {
		return nil, err
	}

	err = h.dumpBondDetails(ifaces)
	if err != nil {
		return nil, err
	}

	// Rx-placement dump is last since it uses interface type-specific data
	err = h.dumpRxPlacement(ifaces)
	if err != nil {
		return nil, err
	}
