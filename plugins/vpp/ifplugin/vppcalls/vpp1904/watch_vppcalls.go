...
import (
	...
	vpp_dhcp "go.ligato.io/vpp-agent/v3/plugins/vpp/binapi/vpp1904/dhcp"
...
func (h *InterfaceVppHandler) WatchDHCPLeases(ctx context.Context, leasesCh chan<- *vppcalls.Lease) error {
	...
	go func() {
		...
		defer h.log.Debug("done watching DHCP lease", ctx.Err())
...
func toDHCPLease(dhcpEvent *vpp_dhcp.DHCPComplEvent) *vppcalls.Lease {
	...
	return &vppcalls.Lease{
		...
		State:         lease.State,
		Hostname:      string(bytes.TrimRight(lease.Hostname, "\x00")),
