...
type FlowProbeParamsDescriptorAdapter struct {
	descriptor *FlowProbeParamsDescriptor
}

func NewFlowProbeParamsDescriptor(typedDescriptor *FlowProbeParamsDescriptor) *KVDescriptor {
	adapter := &FlowProbeParamsDescriptorAdapter{descriptor: typedDescriptor}
...
func (da *FlowProbeParamsDescriptorAdapter) ValueComparator(key string, oldValue, newValue proto.Message) bool {
	typedOldValue, err1 := castFlowProbeParamsValue(key, oldValue)
	typedNewValue, err2 := castFlowProbeParamsValue(key, newValue)
...
func (da *FlowProbeParamsDescriptorAdapter) Validate(key string, value proto.Message) (err error) {
	typedValue, err := castFlowProbeParamsValue(key, value)
	if err != nil {
		return err
	}
...
func (da *FlowProbeParamsDescriptorAdapter) Update(key string, oldValue, newValue proto.Message, oldMetadata Metadata) (newMetadata Metadata, err error) {
	oldTypedValue, err := castFlowProbeParamsValue(key, oldValue)
	if err != nil {
		return nil, err
	}
	newTypedValue, err := castFlowProbeParamsValue(key, newValue)
	if err != nil {
		return nil, err
	}
	typedOldMetadata, err := castFlowProbeParamsMetadata(key, oldMetadata)
	if err != nil {
		return nil, err
	}
...
func (da *FlowProbeParamsDescriptorAdapter) Delete(key string, value proto.Message, metadata Metadata) error {
	...
	typedMetadata, err := castFlowProbeParamsMetadata(key, metadata)
	if err != nil {
		return err
	}
...
func (da *FlowProbeParamsDescriptorAdapter) Retrieve(correlate []KVWithMetadata) ([]KVWithMetadata, error) {
	var correlateWithType []FlowProbeParamsKVWithMetadata
	for _, kvpair := range correlate {
		typedValue, err := castFlowProbeParamsValue(kvpair.Key, kvpair.Value)
		if err != nil {
			continue
		}
		typedMetadata, err := castFlowProbeParamsMetadata(kvpair.Key, kvpair.Metadata)
		if err != nil {
			continue
		}
		correlateWithType = append(correlateWithType,
			FlowProbeParamsKVWithMetadata{
				Key:      kvpair.Key,
				Value:    typedValue,
...
func castFlowProbeParamsValue(key string, value proto.Message) (*vpp_ipfix.FlowProbeParams, error) {
	typedValue, ok := value.(*vpp_ipfix.FlowProbeParams)
	if !ok {
...
func castFlowProbeParamsMetadata(key string, metadata Metadata) (interface{}, error) {
	if metadata == nil {
		return nil, nil
	}
