...
import (
	...
	vpp_ipfix "go.ligato.io/vpp-agent/v3/plugins/vpp/binapi/vpp1904/ipfix_export"
...
//
// Caution: VPP 19.04 does not support IPv6 addresses for IPFIX configuration,
...
func (h *IpfixVppHandler) DumpExporters() ([]*ipfix.IPFIX, error) {
	...
	for {
		...
		collectorIPAddr := net.IP(details.CollectorAddress[:4]).To4()
		sourceIPAddr := net.IP(details.SrcAddress[:4]).To4()

		ipfixes = append(ipfixes,
			&ipfix.IPFIX{
				Collector: &ipfix.IPFIX_Collector{
					Address: collectorIPAddr.String(),
					Port:    uint32(details.CollectorPort),
				},
				SourceAddress:    sourceIPAddr.String(),
				VrfId:            details.VrfID,
