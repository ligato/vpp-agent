...
import (
	...
	"github.com/ligato/vpp-agent/plugins/vpp/srplugin/vppcalls/vpp1901"
	...
	cases := []struct {
	...
	}{
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&sr.SrPolicyMod{
					BsidAddr:  sidA.Addr,
					Operation: vpp1901.DeleteSRList,
					SlIndex:   111,
					FibTable:  10,
					Sids: sr.Srv6SidList{
						Weight:  1,
		...
		},
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&sr.SrSteeringAddDel{
					...
					BsidAddr:      sidA.Addr,
					SrPolicyIndex: uint32(0),
					TrafficType:   vpp1901.SteerTypeL2,
					SwIfIndex:     swIndexA,
				}))
			},
		},
...
func setup(t *testing.T) (*vppcallmock.TestCtx, vppcalls.SRv6VppAPI) {
	...
	vppCalls := vpp1901.NewSRv6VppHandler(ctx.MockChannel, swIfIndex, log)
	return ctx, vppCalls
}
