...
import (
	...
	vpp2106 "go.ligato.io/vpp-agent/v3/plugins/vpp/srplugin/vppcalls/vpp2106"
	...
	cases := []struct {
	...
	}{
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&vpp_sr.SrPolicyMod{
					BsidAddr:  sidA,
					Operation: vpp2106.DeleteSRList,
					SlIndex:   111,
					FibTable:  10, // installationVrfId
					Sids: vpp_sr.Srv6SidList{
						Weight:  1,
	...
	}{
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&vpp_sr.SrSteeringAddDel{
					...
					TableID:       10,
					TrafficType:   vpp2106.SteerTypeIPv6,
		...
		},
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&vpp_sr.SrSteeringAddDel{
					...
					SrPolicyIndex: uint32(0),
					TableID:       10,
					TrafficType:   vpp2106.SteerTypeIPv4,
		...
		},
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&vpp_sr.SrSteeringAddDel{
					...
					BsidAddr:      sidA,
					SrPolicyIndex: uint32(0),
					TrafficType:   vpp2106.SteerTypeL2,
					SwIfIndex:     swIndexA,
				}))
			},
		},
...
func setup(t *testing.T) (*vppmock.TestCtx, vppcalls.SRv6VppAPI) {
	...
	vppCalls := vpp2106.NewSRv6VppHandler(ctx.MockVPPClient, swIfIndex, log)
	return ctx, vppCalls
}
...
func toAddress(ip interface{}) (addr ip_types.Address) {
	switch ip := ip.(type) {
	case string:
		addr, _ = vpp2106.IPToAddress(ip)
	case net.IP:
		addr, _ = vpp2106.IPToAddress(ip.String())
	default:
