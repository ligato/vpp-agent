...
import (
	...
	"github.com/ligato/vpp-agent/plugins/vpp/srplugin/vppcalls/vpp1904"
	...
	}{
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&sr.SrSteeringAddDel{
					...
					TableID:       10,
					TrafficType:   vpp1904.SteerTypeIPv6,
		...
		},
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&sr.SrSteeringAddDel{
					...
					SrPolicyIndex: uint32(0),
					TableID:       10,
					TrafficType:   vpp1904.SteerTypeIPv4,
		...
		},
		{
			...
			Verify: func(err error, catchedMsg govppapi.Message) {
				...
				Expect(catchedMsg).To(Equal(&sr.SrSteeringAddDel{
					...
					BsidAddr:      sidA.Addr,
					SrPolicyIndex: uint32(0),
					TrafficType:   vpp1904.SteerTypeL2,
					SwIfIndex:     swIndexA,
				}))
			},
		},
...
func setup(t *testing.T) (*vppcallmock.TestCtx, vppcalls.SRv6VppAPI) {
	...
	vppCalls := vpp1904.NewSRv6VppHandler(ctx.MockChannel, swIfIndex, log)
	return ctx, vppCalls
}
