...
type SRv6Calls interface {
	...
	AddLocalSid(sidAddr net.IP, localSID *srv6.LocalSID, swIfIndex ifaceidx.SwIfIndex, vppChan govppapi.VPPChannel) error
	// DeleteLocalSid delets local sid given by <sidAddr> in VPP
	DeleteLocalSid(sidAddr net.IP, vppChan govppapi.VPPChannel) error
	// SetEncapsSourceAddress sets for SRv6 in VPP the source address used for encapsulated packet
	SetEncapsSourceAddress(address string, vppChan govppapi.VPPChannel) error
	...
	AddPolicy(bindingSid net.IP, policy *srv6.Policy, policySegment *srv6.PolicySegment, vppChan govppapi.VPPChannel) error
	// DeletePolicy deletes SRv6 policy given by binding SID <bindingSid>
	DeletePolicy(bindingSid net.IP, vppChan govppapi.VPPChannel) error
	// AddPolicySegment adds segment <policySegment> to SRv6 policy <policy> that has policy BSID <bindingSid>
	AddPolicySegment(bindingSid net.IP, policy *srv6.Policy, policySegment *srv6.PolicySegment, vppChan govppapi.VPPChannel) error
	...
	DeletePolicySegment(bindingSid net.IP, policy *srv6.Policy, policySegment *srv6.PolicySegment, segmentIndex uint32, vppChan govppapi.VPPChannel) error
	// AddSteering sets in VPP steering into SRv6 policy.
	AddSteering(steering *srv6.Steering, swIfIndex ifaceidx.SwIfIndex, vppChan govppapi.VPPChannel) error
	// RemoveSteering removes in VPP steering into SRv6 policy.
	RemoveSteering(steering *srv6.Steering, swIfIndex ifaceidx.SwIfIndex, vppChan govppapi.VPPChannel) error
}
...
// AddLocalSid adds local sid given by <sidAddr> and <localSID> into VPP
func (calls *srv6Calls) AddLocalSid(sidAddr net.IP, localSID *srv6.LocalSID, swIfIndex ifaceidx.SwIfIndex, vppChan govppapi.VPPChannel) error {
	return calls.addDelLocalSid(false, sidAddr, localSID, swIfIndex, vppChan)
}

// DeleteLocalSid delets local sid given by <sidAddr> in VPP
func (calls *srv6Calls) DeleteLocalSid(sidAddr net.IP, vppChan govppapi.VPPChannel) error {
	return calls.addDelLocalSid(true, sidAddr, nil, nil, vppChan)
}

func (calls *srv6Calls) addDelLocalSid(deletion bool, sidAddr net.IP, localSID *srv6.LocalSID, swIfIndex ifaceidx.SwIfIndex, vppChan govppapi.VPPChannel) error {
...
// SetEncapsSourceAddress sets for SRv6 in VPP the source address used for encapsulated packet
func (calls *srv6Calls) SetEncapsSourceAddress(address string, vppChan govppapi.VPPChannel) error {
...
// AddPolicy adds SRv6 policy given by identified <bindingSid>,initial segment for policy <policySegment> and other policy settings in <policy>
func (calls *srv6Calls) AddPolicy(bindingSid net.IP, policy *srv6.Policy, policySegment *srv6.PolicySegment, vppChan govppapi.VPPChannel) error {
...
// DeletePolicy deletes SRv6 policy given by binding SID <bindingSid>
func (calls *srv6Calls) DeletePolicy(bindingSid net.IP, vppChan govppapi.VPPChannel) error {
...
// AddPolicySegment adds segment <policySegment> to SRv6 policy <policy> that has policy BSID <bindingSid>
func (calls *srv6Calls) AddPolicySegment(bindingSid net.IP, policy *srv6.Policy, policySegment *srv6.PolicySegment, vppChan govppapi.VPPChannel) error {
...
func (calls *srv6Calls) DeletePolicySegment(bindingSid net.IP, policy *srv6.Policy, policySegment *srv6.PolicySegment,
	segmentIndex uint32, vppChan govppapi.VPPChannel) error {
...
func (calls *srv6Calls) AddSteering(steering *srv6.Steering, swIfIndex ifaceidx.SwIfIndex,
	vppChan govppapi.VPPChannel) error {
...
func (calls *srv6Calls) addDelSteering(delete bool, steering *srv6.Steering, swIfIndex ifaceidx.SwIfIndex, vppChan govppapi.VPPChannel) error {
	defer func(t time.Time) {
