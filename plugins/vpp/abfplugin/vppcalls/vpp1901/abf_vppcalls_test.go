...
func TestAddABFPolicy(t *testing.T) {
	...
	err := abfHandler.AddAbfPolicy(1, 2, []*vpp_abf.ABF_ForwardingPath{
		{
			InterfaceName: "if1",
			NextHopIp:     "10.0.0.1",
			LabelStack: []*vpp_abf.ABF_ForwardingPath_Label{
				{
					Label: 7,
				},
			},
	...
	Expect(req.Policy.Paths[0].NextHop).To(BeEquivalentTo(net.ParseIP("10.0.0.1").To4()))
	Expect(req.Policy.Paths[0].LabelStack[0].Label).To(BeEquivalentTo(uint32(7)))
