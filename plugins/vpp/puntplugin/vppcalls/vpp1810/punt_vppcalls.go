...
func (h *PuntVppHandler) RegisterPuntSocket(puntCfg *punt.ToHost) (string, error) {
	...
	path4, err := h.registerPuntWithSocketIPv4(puntCfg)
	if err != nil {
		return "", err
	}
	path6, err := h.registerPuntWithSocketIPv6(puntCfg)
	if err != nil {
		return "", err
	}

	if path4 != path6 {
		h.log.Warnf("returned IPv4 Pathname (%s) for registered punt socket does not match IPv6 Pathname (%s)",
			path4, path6)
	}
...
func (h *PuntVppHandler) DeregisterPuntSocket(puntCfg *punt.ToHost) error {
	if puntCfg.L3Protocol == punt.L3Protocol_IPv4 {
		return h.unregisterPuntWithSocketIPv4(puntCfg)
	} else if puntCfg.L3Protocol == punt.L3Protocol_IPv6 {
		return h.unregisterPuntWithSocketIPv6(puntCfg)
	}
	// if L3 is set to all, deregister both, IPv4 and IPv6
	err := h.unregisterPuntWithSocketIPv4(puntCfg)
	if err != nil {
		return err
	}
	return h.unregisterPuntWithSocketIPv6(puntCfg)
...
func (h *PuntVppHandler) registerPuntWithSocketIPv4(punt *punt.ToHost) (string, error) {
	return h.registerPuntWithSocket(punt, true)
}

func (h *PuntVppHandler) registerPuntWithSocketIPv6(punt *punt.ToHost) (string, error) {
	return h.registerPuntWithSocket(punt, false)
}

func (h *PuntVppHandler) registerPuntWithSocket(punt *punt.ToHost, isIPv4 bool) (string, error) {
