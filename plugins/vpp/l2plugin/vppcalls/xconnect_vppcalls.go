...
func AddL2XConnect(rxIfIdx uint32, txIfIdx uint32, vppChan govppapi.VPPChannel, stopwatch *measure.Stopwatch) error {
	return addDelXConnect(rxIfIdx, txIfIdx, true, vppChan, stopwatch)
}

// DeleteL2XConnect removes xConnect between two interfaces.
func DeleteL2XConnect(rxIfIdx uint32, txIfIdx uint32, vppChan govppapi.VPPChannel, stopwatch *measure.Stopwatch) error {
	return addDelXConnect(rxIfIdx, txIfIdx, false, vppChan, stopwatch)
}

func addDelXConnect(rxIfaceIdx uint32, txIfaceIdx uint32, enable bool, vppChan govppapi.VPPChannel, stopwatch *measure.Stopwatch) error {
