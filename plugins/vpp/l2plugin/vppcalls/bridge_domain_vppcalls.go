...
func VppAddBridgeDomain(bdIdx uint32, bd *l2.BridgeDomains_BridgeDomain, vppChan govppapi.VPPChannel, stopwatch *measure.Stopwatch) error {
...
func VppDeleteBridgeDomain(bdIdx uint32, vppChan govppapi.VPPChannel, stopwatch *measure.Stopwatch) error {
	defer func(t time.Time) {
		stopwatch.TimeLog(l2ba.BridgeDomainAddDel{}).LogTimeEntry(time.Since(t))
	}(time.Now())
