...
type InterfaceVrfDescriptorAdapter struct {
	descriptor *InterfaceVrfDescriptor
}

func NewInterfaceVrfDescriptor(typedDescriptor *InterfaceVrfDescriptor) *KVDescriptor {
	adapter := &InterfaceVrfDescriptorAdapter{descriptor: typedDescriptor}
...
func (da *InterfaceVrfDescriptorAdapter) ValueComparator(key string, oldValue, newValue proto.Message) bool {
	typedOldValue, err1 := castInterfaceVrfValue(key, oldValue)
	typedNewValue, err2 := castInterfaceVrfValue(key, newValue)
...
func (da *InterfaceVrfDescriptorAdapter) Validate(key string, value proto.Message) (err error) {
	typedValue, err := castInterfaceVrfValue(key, value)
	if err != nil {
		return err
	}
...
func (da *InterfaceVrfDescriptorAdapter) Update(key string, oldValue, newValue proto.Message, oldMetadata Metadata) (newMetadata Metadata, err error) {
	oldTypedValue, err := castInterfaceVrfValue(key, oldValue)
	if err != nil {
		return nil, err
	}
	newTypedValue, err := castInterfaceVrfValue(key, newValue)
	if err != nil {
		return nil, err
	}
	typedOldMetadata, err := castInterfaceVrfMetadata(key, oldMetadata)
	if err != nil {
		return nil, err
	}
...
func (da *InterfaceVrfDescriptorAdapter) Delete(key string, value proto.Message, metadata Metadata) error {
	...
	typedMetadata, err := castInterfaceVrfMetadata(key, metadata)
	if err != nil {
		return err
	}
...
func (da *InterfaceVrfDescriptorAdapter) Retrieve(correlate []KVWithMetadata) ([]KVWithMetadata, error) {
	var correlateWithType []InterfaceVrfKVWithMetadata
	for _, kvpair := range correlate {
		typedValue, err := castInterfaceVrfValue(kvpair.Key, kvpair.Value)
		if err != nil {
			continue
		}
		typedMetadata, err := castInterfaceVrfMetadata(kvpair.Key, kvpair.Metadata)
		if err != nil {
			continue
		}
		correlateWithType = append(correlateWithType,
			InterfaceVrfKVWithMetadata{
				Key:      kvpair.Key,
				Value:    typedValue,
...
func castInterfaceVrfValue(key string, value proto.Message) (*linux_interfaces.Interface, error) {
...
func castInterfaceVrfMetadata(key string, metadata Metadata) (interface{}, error) {
	if metadata == nil {
		return nil, nil
	}
