...
func TestIPSecTunnelKey(t *testing.T) {
	tests := []struct {
		name        string
		tunName     string
		expectedKey string
		...
		{
			name:        "valid IPSec tunnel name",
			tunName:     "tun1",
			expectedKey: "vpp/config/v2/ipsec/tunnel/tun1",
		},
		{
			name:        "invalid IPSec tunnel name",
			tunName:     "",
			expectedKey: "vpp/config/v2/ipsec/tunnel/<invalid>",
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			key := ipsec.TunnelKey(test.tunName)
			if key != test.expectedKey {
				t.Errorf("failed for: tunName=%s\n"+
					"expected key:\n\t%q\ngot key:\n\t%q",
					test.name, test.expectedKey, key)
			}
		})
...
func TestParseIPSecTunnelNameFromKey(t *testing.T) {
	tests := []struct {
		name                string
		key                 string
		expectedTunnelName  string
		expectedIsTunnelKey bool
	}{
		{
			name:                "valid IPSec tunnel name",
			key:                 "vpp/config/v2/ipsec/tunnel/tunnel1",
			expectedTunnelName:  "tunnel1",
			expectedIsTunnelKey: true,
		},
		{
			name:                "invalid IPSec tunnel name",
			key:                 "vpp/config/v2/ipsec/tunnel/<invalid>",
			expectedTunnelName:  "<invalid>",
			expectedIsTunnelKey: true,
		},
		{
			name:                "not IPSec tunnel key",
			key:                 "vpp/config/v2/ipsec/spd/tunnel1",
			expectedTunnelName:  "",
			expectedIsTunnelKey: false,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			tunnelName, isTunnelKey := ipsec.ParseTunnelNameFromKey(test.key)
			if isTunnelKey != test.expectedIsTunnelKey {
				t.Errorf("expected isTunnelKey: %v\tgot: %v", test.expectedIsTunnelKey, isTunnelKey)
			}
			if tunnelName != test.expectedTunnelName {
				t.Errorf("expected tunnelName: %s\tgot: %s", test.expectedTunnelName, tunnelName)
			}
		})
	}
}
